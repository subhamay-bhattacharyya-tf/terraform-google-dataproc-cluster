...
### Dataproc Cluster Module — Annotated Example

```hcl
# -----------------------------------------------------------------------------
# modules/dataproc-cluster/main.tf
# -----------------------------------------------------------------------------
...
# Core Dataproc cluster resource
resource "google_dataproc_cluster" "this" {
  name    = local.cluster_name
  project = var.dataproc_cluster_config.project_id
  region  = var.region
  labels  = var.dataproc_cluster_config.labels

  cluster_config {
    # Software image version (e.g. "2.1-debian11", "2.2-debian12")
    software_config {
    ...
    # Master node — 1 for standard, 3 for HA
    master_config {
    ...
    # Preemptible/spot workers — conditional on count > 0
    ...
    # Network placement — only set when network or subnetwork is provided
    ...
    # Web UI access (Yarn, Spark, etc.) — conditional
    ...
    # Auto-delete when idle — conditional on idle_delete_ttl being set
...
### Dataproc Machine Types Quick Reference

| Machine Type | vCPUs | Memory | Use Case |
|---|---|---|---|
| `n1-standard-4` | 4 | 15 GB | General purpose (default) |
| `n1-standard-8` | 8 | 30 GB | Medium workloads |
| `n1-highmem-4` | 4 | 26 GB | Memory-intensive (Spark shuffle) |
| `n1-highmem-8` | 8 | 52 GB | Heavy in-memory processing |
| `n1-highcpu-8` | 8 | 7.2 GB | CPU-bound workloads |

### Optional Components

| Component | Description |
|---|---|
| `JUPYTER` | JupyterLab notebook server |
| `ZEPPELIN` | Apache Zeppelin notebook |
| `HIVE_WEBHCAT` | WebHCat REST API for Hive |
| `PRESTO` | Presto distributed SQL engine |
